...
module Network.Nakadi.Subscriptions.Events
  ( subscriptionProcessConduit,
    subscriptionProcessConduitSync,
    subscriptionProcess,
    subscriptionProcessSync,
    subscriptionSource,
    subscriptionSourceEvents,
    subscriptionSourceSync,
    subscriptionSourceEventsSync,
  )
where
...
subscriptionProcess subscriptionId processor = subscriptionProcessConduit subscriptionId conduit
  where
    conduit = iterMC processor

-- | Experimental API.
--
-- Synchronous version of `subscriptionProcess`.
subscriptionProcessSync ::
  (MonadNakadi b m, MonadUnliftIO m, MonadMask m, MonadResource m, FromJSON a) =>
  -- | Subscription to consume
  SubscriptionId ->
  ...
  (SubscriptionEventStreamBatch a -> m ()) ->
  m ()
subscriptionProcessSync subscriptionId processor =
  subscriptionProcessConduitSync subscriptionId (iterMC processor)
...
subscriptionProcessConduitSync ::
  ...
  ConduitM batch batch m () ->
  m ()
subscriptionProcessConduitSync subscriptionId processor = do
  config <- nakadiAsk
  let queryParams = buildConsumeQueryParameters config
  httpJsonBodyStream
    ok200
    [(status404, errorSubscriptionNotFound)]
    (includeFlowId config . setRequestPath path . setRequestQueryParameters queryParams)
    $ subscriptionProcessHandlerSync subscriptionId processor
  where
    path = "/subscriptions/" <> subscriptionIdToByteString subscriptionId <> "/events"
...
subscriptionProcessHandler ::
  ...
  -- | Subscription ID required for committing.
  SubscriptionId ->
  -- | User provided Conduit for stream.
  ConduitM batch batch m () ->
  -- | Streaming response from Nakadi
...
-- | This function processes a subscription synchronously, committing after every batch.
subscriptionProcessHandlerSync ::
  forall a b m batch.
  ( MonadNakadi b m,
    MonadUnliftIO m,
    MonadResource m,
  ...
  Response (ConduitM () ByteString m ()) ->
  m ()
subscriptionProcessHandlerSync subscriptionId processor response = do
  eventStream <- buildSubscriptionEventStream subscriptionId response
  runConduit $
    responseBody response
      .| linesUnboundedAsciiC
...
subscriptionSourceSync ::
  ...
  -- | Conduit source.
  ConduitM () (SubscriptionEventStreamBatch a) m ()
subscriptionSourceSync subscriptionId = do
  UnliftIO {..} <- lift askUnliftIO
  streamLimit <- lift nakadiAsk <&> (fmap fromIntegral . view L.streamLimit)
  queue <- atomically $ newTBMQueue 1
  (_releaseKey, asyncHandle) <-
    allocate
      (unliftIO (async (subscriptionConsumer streamLimit queue)))
      cancel
  ...
  where
    ...
    subscriptionConsumer maybeStreamLimit queue = go `finally` atomically (closeTBMQueue queue)
      where
        go = do
          subscriptionProcessSync subscriptionId (void . atomically . writeTBMQueue queue)
          -- We only reconnect automatically when no stream limit is set.
          -- This effectively means that we don't try to reach @streamLimit@ events exactly.
          -- We simply regard @streamLimit@ as an upper bound and in case Nakadi disconnects us
          -- earlier or the connection breaks, we produce less than @streamLimit@ events.
...
-- | Experimental API.
--
-- Synchronous version of `subscriptionSourceEvents`.
subscriptionSourceEventsSync ::
  (MonadNakadi b m, MonadUnliftIO m, MonadMask m, MonadResource m, FromJSON a) =>
  -- | Subscription to consume
  SubscriptionId ->
  -- | Conduit processor.
  ConduitM () a m ()
subscriptionSourceEventsSync subscriptionId =
  subscriptionSourceSync subscriptionId .| concatMapC (\batch -> fromMaybe mempty (batch & _events))
